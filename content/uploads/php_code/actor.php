...
if($mode == 'add') {
	...
	if($mode == 'add' && $_POST['submit']) {
		$name = $_POST['name'];
		$info = RemoveHack($_POST['info']);
		$urlmore = RemoveHack($_POST['urlmore']);
		$thumb = RemoveHack($_POST['thumb']);
		MySql::dbinsert('actor',"name,info,urlmore,thumb","'$name','$info','$urlmore','$thumb'");
		header("Location: ?action=actor&mode=add");
	}
?>
		<div class="widget stacked">
			<div class="widget-header">
				<i class="icon-check"></i>
				<h3>Thêm Nhóm dịch</h3>
			</div>
			<!-- /widget-header -->
