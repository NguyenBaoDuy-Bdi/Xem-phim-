 ...
 if(!$_POST['one']) { ?>
				...
				<form class="form-horizontal" method="post" enctype="multipart/form-data"/>
					<fieldset>
						...
						<div class="control-group">
							...
							<div class="controls">
								<textarea id="multi" name="multi" class="span8" rows="10" placeholder="Tên tập | Liên kết‏"></textarea>
							</div>
						</div>
			...
			<div class="widget-content">
				<form class="form-horizontal" method="post" enctype="multipart/form-data"/>
					<fieldset>
						...
						<div class="control-group">
							<label class="control-label" for="info<?php echo $i;?>">Thông tin</label>
							<div class="controls">
								<input type="text" class="input-large" name="info[<?php echo $i;?>]" id="info<?php echo $i;?>" value="<?php echo $info;?>"/>
							</div>
						</div>
						<div class="control-group">
			...
			<?php } else {
				for($i=$_POST['begin'];$i<=$_POST['end'];$i++) {
					...
					$thumb = $_POST['thumb'][$i];
					if($Uploadgroup == '3' && $name && $url) MySql::dbinsert("episode","filmid,name,url,thumb,userpost,present,download","'$filmid','$name','$url','$thumb','$Uploadid','$present','$download'");
					$timeupdate = time();
