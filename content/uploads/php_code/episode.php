...
if($mode == 'add' || $mode == 'edit') {
	if($mode == 'edit') {
		$arr = MySql::dbselect('id,name,url,subtitle,thumb,filmid,userpost,default_subtitle_id,present,download','episode',"id = '$epid' AND userpost = '$upload_id'");
                $subtitles = MySql::dbselect('subtitle_url,subtitle_lang,id','subtitle',"episode_id = '$epid'"); 
		$filmid = $arr[0][5];
		$name = $arr[0][1];
		$info = $arr[0][8];
		$download = $arr[0][9];
		if ($arr[0][6] == $upload_id or $_SESSION["RK_Uploadgroup"] == '3')
			{
			$url = $arr[0][2];
			}
	...
	if($mode == 'add' && $_POST['submit'] && ($Uploadgroup == '3')) {
		...
		MySql::dbinsert("episode","filmid,name,url,thumb,subtitle,userpost,datetime_post,present,download","'$filmid','$name','$url','$thumb','$subtitle','$upload_id','$date12','$present','$download'");
	...
	}else if($mode == 'edit' && $_POST['submit'] && ($Uploadgroup == '3' or $arr[0][6] == $admin_id)) {
		$filmid = RemoveHack($_POST['filmid']);
		$name = RemoveHack($_POST['name']);
		$info = RemoveHack($_POST['info']);
		$url = RemoveHack($_POST['url']);
					...
					<fieldset>
						<div class="control-group">
							...
							<div class="controls">
								...
								<?php echo upload_film($filmid,$userid);?>
								</select>
							</div>
						</div>
						<div class="control-group">
							<label class="control-label" for="name">Tên tập</label>
							<div class="controls">
								<input type="text" class="input-large" name="name" id="name" value="<?php echo $name;?>"/>
							</div>
						</div>
						<div class="control-group">
							<label class="control-label" for="name">Thông tin</label>
							<div class="controls">
								<input type="text" class="input-large" name="info" id="info" value="<?php echo $info;?>"/>
							</div>
						</div>
						<div class="control-group">
						...
						<div class="control-group">
							<label class="control-label" for="download">Link Download</label>
							<div class="controls">
						...
						<div class="control-group">
							...
							<div class="controls">
						<input type="radio" class="input-large" name="present" id="present" value="1" <?if($present==1){?>checked<?}?> /> Thuyết minh<br/>
						<input type="radio" class="input-large" name="present" id="present" value="2" <?if($present==2){?>checked<?}?> /> Vietsub<br/>
						<input type="radio" class="input-large" name="present" id="present" value="3" <?if($present==3){?>checked<?}?> /> Vietsub Full
							</div>
						</div>
...
$limit 		= 	($page-1)*$num;
if($limit<0) 	$limit=0;
$arr = MySql::dbselect('id,name,url,subtitle,userpost,active,present','episode',"filmid = '$filmid' AND userpost = '$upload_id' order by id desc LIMIT $limit,$num");
			...
			<div class="widget-content">
				...
				<?php
					for($i=0;$i<count($arr);$i++) {
						$id = $arr[$i][0];
						$name = $arr[$i][1];
						if ($arr[$i][4] == $upload_id or $_SESSION["RK_Uploadgroup"] == '3')
							{
