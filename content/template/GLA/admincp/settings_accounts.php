...
if(isset($_POST['update'])) {
    foreach($_POST as $key => $value) {
        if(isset($config[$key])) {
            ...
            # Check if we should update the cloudflare settings
            # If yes, then we validate them before saving them
           $config_class->update($key, $value);
        }
    }
    $msg->add('s', '<strong>Success!</strong> The data has been successfully updated.', 'settings_accounts.php');
}

# Render template
$template = $twig->loadTemplate('settings_accounts.html.twig');
