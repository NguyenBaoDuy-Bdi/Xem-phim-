...
if($mode == 'delete' && $Admingroup == '2') {
	...
	if($mode == 'edit' && $_POST['submit'] && $Admingroup == '2') {
		...
		$ugroup = RemoveHack($_POST['ugroup']);
		$fullname = RemoveHack($_POST['fullname']);
		MySql::dbinsert("user","username,email,ugroup,fullname","'$username','$email','$ugroup','$fullname'");
		header("Location: ?action=user");
				...
				<form class="form-horizontal" method="post" enctype="multipart/form-data"/>
					<fieldset>
						...
						<div class="control-group">
							...
							<div class="controls">
								<input type="text" class="input-large" name="password" id="password"/> (Nhập nếu nuốn đổi)
							</div>
						</div>
						<div class="control-group">
...
$arr = MySql::dbselect('id,username,email,ugroup','user',"$sql order by id desc LIMIT $limit,$num");
...
$(function () {
	$('.deletefilm').live('click', function (e) {
		...
		$.msgbox("Bạn có chắc muốn xóa thành viên này?", {
		type: "confirm",
		buttons : [
			...
			<div class="widget-content">
				...
				<?php
					for($i=0;$i<count($arr);$i++) {
						...
						$views = one_data("SUM(viewed) views","film","userpost = '".$id."'");
				?>
				<tr>
					<td>
					...
					<td>
						<?php echo one_data("COUNT(id)","film","userpost = '".$id."'");?>  phim
					</td>
					<td>
						<?php echo number_format($views*(3000/1000));?> <font color="red">đ </font>
					</td>
					<td>
						<?php echo $ugroup;?>
					</td>
