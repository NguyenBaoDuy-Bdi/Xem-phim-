...
function picasa($link) {        
	...
	if($v1080p != '' and (strpos($v1080p, 'redirector')  !== false)){
	...
	} elseif($v720p != '' and (strpos($v720p, 'redirector')  !== false)){       
			$js .= '<source src="'.$v720p.'" type="video/mp4" data-res="720"/>';        
			$js .= '<source src="'.$v360p.'" type="video/mp4" data-res="360"/>'; 			
	} else {        
			$js .= '<source src="'.$v360p.'" type="video/mp4" data-res="360"/>';    
...
$cache = phpFastCache();
$products = $cache->get($url);
if($products == null) {
	$products = get_curl($url);
	$cache->set($url,$products , 1728000);
	echo "";
} else {
	echo "";
	echo $products;
...
function player($epid,$type='') {
	...
	$linkget = '<video id="myplayer" class="video-js vjs-default-skin" controls autoplay="autoplay" width="100%" height="100%" poster="http://hayphimhd.com/content/template/images/cover.jpg" data-setup=\'{ "techOrder": ["youtube"], "src": "'.$url.'" }\'>';
	}*/
	...
	*/
	...
	if($linkget != ''){
		$mplayer = '<video id="myplayer" class="video-js vjs-default-skin" controls autoplay="autoplay" width="100%" height="100%" poster="http://hayphimhd.com/content/template/images/cover.jpg" data-setup="">
		
					  '.$linkget.'
					  <track src="/players/subwcome.srt" kind="subtitles" srclang="vi" label="Giới Thiệu" default>
