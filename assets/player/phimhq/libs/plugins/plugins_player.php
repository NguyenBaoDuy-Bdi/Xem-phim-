<?php
function decrypt_at($url){
	if(strpos($url,'picasa')){
		$url = str_replace('https://picasaweb.google.com/lh/photo/','',$url);
		$url = base64_decode($url);
	}
	return $url;
}
$link = decrypt_at($_POST['url']);
...
if($link){
	...
	if(strpos($link,"picasaweb.google.com/")){
		if(stripos($link,'#')){
			$arr = explode('#',$link);
			$arr = explode('"gphoto$id":"'.$arr[1],$text);
			$arr = explode('}],"title":"',$arr[1]);
			$arr = explode('"media":{"',$arr[0]);
			if(!empty($arr[1])){
				$text = $arr[1];
			}
		}
		else{
			$arr = explode('content":[{"',$text);
			$arr = explode('}],"title":"',$arr[1]);
			if(!empty($arr[0])){
				$text = 'content":[{"'.$arr[0];
			}
