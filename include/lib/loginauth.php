...
class LoginAuth {
	...
	public static function addUser($username,$password,$confirmpass,$email,$captcha) {
		...
		if(MySql::dbselect("id","user","username = '$username'")) $arr = '{"status":0,"messages":{"email":"Tài khoản đã được sử dụng."}}';
		else if(MySql::dbselect("id","user","email = '$email'")) $arr = '{"status":0,"messages":{"email":"Email đã được sử dụng."}}';
		else if($password !== $confirmpass) $arr = '{"status":0,"messages":{"email":"Xác nhận mật khẩu không đúng."}}';
		else if(strlen($password) < 6 OR strlen($password) > 15) $arr = '{"status":0,"messages":{"email":"Mật khẩu không hợp lệ."}}';
	...
	public static function loginUser($username,$password,$remember) {
		...
		if(!$user) $arr = '{"status":0,"message":"Username is invalid."}';
		else if($upassword !== $password) $arr = '{"status":0,"message":"Password is invalid."}';
		else {
			$userid = $user[0][0];
			$lastlogin = time();
			...
			$arr = '{"status":1,"message":"Login Success."}';
			//header("Location: ".SITE_URL);
		}
		return $arr;
	}
